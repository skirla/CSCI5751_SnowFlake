...
select top 10 c.CustomerID as cust_id, c.LastName as cust_last_name, c.FirstName as cust_first_name, sum(p.price) tot_amnt_purchased
from raw.sales s, raw.customers c,  raw.products p
where s.CustomerID = c.CustomerID
and s.ProductID = p.ProductID
...
,p.name as "Product Name", p.price as "Product Price", s.Quantity , (select sum(p2.price * p2.quantity) from raw.products p2) as "Total Sales Amount"
